...
class JPGToTIFFPlugin(ConverterPlugin):

    slug = "jpg-to-tiff"

    name = "JPG to TIFF"

    description = "Convert JPG images to TIFF format for archival and printing."
    ...
    async def convert(self, source_path: Path, target_format: str) -> Path:
        ...
        if not self.supports(source_path.suffix, target_format):
            raise RuntimeError("JPGToTIFFPlugin only supports JPG -> TIFF.")
