...
for size_mb in sizes_mb:
    ...
    out_path = TMP / f'{size_mb}mb.mp4'
    print(f'\n=== TEST {size_mb} MB ===')
    cmd=['ffmpeg','-y','-f','lavfi','-i','color=c=black:s=1280x720:d=10','-f','lavfi','-i','anullsrc=r=44100:cl=stereo','-c:v','libx264','-preset','ultrafast','-b:v','4M','-c:a','aac','-b:a','128k','-shortest','-fs',str(target_bytes),str(out_path)]
    t0=time.time()
