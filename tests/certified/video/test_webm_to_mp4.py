...
# This certified test creates a tiny WebM sample at runtime and converts it to
# MP4 using FFmpeg when the runtime is available.
import pytest
...
@pytest.mark.asyncio
async def test_webm_to_mp4(tmp_path: Path) -> None:
    skip_if_ffmpeg_unavailable()

    source_path = await create_sample_video(tmp_path, "sample.webm")
