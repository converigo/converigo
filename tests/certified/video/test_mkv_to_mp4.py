...
# This certified test creates a tiny MKV sample at runtime and converts it to
...
@pytest.mark.asyncio
async def test_mkv_to_mp4(tmp_path: Path) -> None:
    skip_if_ffmpeg_unavailable()

    source_path = await create_sample_video(tmp_path, "sample.mkv")
