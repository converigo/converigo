"""
PROJECT: CONVERIGO
TEST SUITE: Certified DOCX to JPG Converter (PR-A2)
STATUS: DEVELOPMENT (Not yet certified — certification pending evidence review)

Basic test coverage for the DOCX -> JPG converter (first page only).
Pipeline: python-docx -> reportlab PDF -> PyMuPDF -> JPG.
"""
...
def test_docx_to_jpg_plugin_discovered():
    """TEST 001: Plugin is properly registered."""
    plugin = registry.get_plugin("docx", "jpg", slug="docx-to-jpg")
    assert plugin is not None
    assert plugin.slug == "docx-to-jpg"
    assert "jpg" in plugin.target_formats or "jpeg" in plugin.target_formats


def test_docx_to_jpg_conversion_success():
    """TEST 002: DOCX converts to JPG successfully."""
    client = TestClient(app)
    response = _convert(client, "sample.docx", "docx-to-jpg")
...
def test_docx_to_jpg_output_exists_and_extension_correct():
...
def test_docx_to_jpg_output_is_valid_jpeg():
...
def test_docx_to_jpg_first_page_only_single_output():
    """TEST 005: Exactly one JPG output file is produced (first page only)."""
    client = TestClient(app)
    response = _convert(client, "sample.docx", "docx-to-jpg")
    assert response.status_code == 201, response.text
...
def test_docx_to_jpg_download_served():
