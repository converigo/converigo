"""
PROJECT: CONVERIGO
TEST SUITE: Certified Audio Factory Converter - wav-to-aac (Phase 21.2 Batch 1)
...
def test_wav_to_aac_plugin_discovered() -> None:
    assert_slug_discovered(SLUG, "wav", "aac")


@pytest.mark.certified
@pytest.mark.asyncio
async def test_wav_to_aac_happy_path(tmp_path: Path) -> None:
    skip_if_ffmpeg_unavailable()
...
@pytest.mark.certified
def test_wav_to_aac_honest_422_for_undecodable_input(tmp_path: Path) -> None:
    """Corrupt bytes must answer the honest UNSUPPORTED_CONVERSION class."""
    bad = tmp_path / "bad.wav"
    bad.write_bytes(b"not-a-real-wav-payload" * 16)
