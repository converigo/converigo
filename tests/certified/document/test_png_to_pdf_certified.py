"""
PROJECT: CONVERIGO
TEST SUITE: Certified PNG to PDF Converter (Batch 2)
STATUS: CERTIFIED (Batch 2 evidence run — pending PC merge-gate approval)

Certified-level coverage for the PNG -> PDF converter
(PNGToPDFPlugin: ReportLab + Pillow, both already in requirements).

Verification: real-file sample (tests/sample.png), plugin-level conversion,
...
SAMPLE_PNG = Path("tests/sample.png")
...
def _make_real_png(tmp_path: Path) -> Path:
    png = tmp_path / "batch2_sample.png"
    Image.new("RGB", (320, 200), (40, 90, 180)).save(png, format="PNG")
    assert png.exists()
    return png


def _convert(client, filename: str = "sample.png"):
    assert SAMPLE_PNG.exists(), f"Sample file is missing: {SAMPLE_PNG}"
    with SAMPLE_PNG.open("rb") as handle:
        return client.post(
            "/convert",
            files={"file": (filename, handle, "image/png")},
            data={"target_format": "pdf", "operation": "png-to-pdf"},
        )


@pytest.mark.certified
def test_png_to_pdf_plugin_discovered():
    """TEST 001: Plugin is properly registered and not a placeholder."""
    plugin = registry.get_plugin("png", "pdf", slug="png-to-pdf")
    assert plugin is not None
    assert plugin.slug == "png-to-pdf"
    assert plugin.engine == "document"
    assert "png" in plugin.source_formats
    assert "pdf" in plugin.target_formats


@pytest.mark.certified
def test_png_to_pdf_plugin_produces_real_pdf(tmp_path: Path) -> None:
    """TEST 002: Plugin produces a real single-page PDF from a PNG sample."""
    plugin = registry.get_plugin("png", "pdf", slug="png-to-pdf")
    src = _make_real_png(tmp_path)
    output = asyncio.run(
...
@pytest.mark.certified
def test_png_to_pdf_conversion_success():
...
@pytest.mark.certified
def test_png_to_pdf_output_is_valid_pdf():
...
@pytest.mark.certified
def test_png_to_pdf_download_served():
...
@pytest.mark.certified
def test_png_to_pdf_single_output_file():
...
@pytest.mark.certified
def test_png_to_pdf_rejects_non_image_input():
